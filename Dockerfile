...
WORKDIR /src
COPY ["E-Wolontariat.csproj", "."]
RUN dotnet restore "./E-Wolontariat.csproj"
COPY . .
WORKDIR "/src/."
RUN dotnet build "./E-Wolontariat.csproj" -c $BUILD_CONFIGURATION -o /app/build

# Ten etap służy do publikowania projektu usługi do skopiowania do etapu końcowego
FROM build AS publish
ARG BUILD_CONFIGURATION=Release
RUN dotnet publish "./E-Wolontariat.csproj" -c $BUILD_CONFIGURATION -o /app/publish /p:UseAppHost=false
